...
class PlayerStatsTable extends StatelessWidget {
  ...
  Widget _buildPlayerRow(BuildContext context, GameTeamPlayerStat stat) {
    final benchImp = _getBenchImp(stat.id);
        ...
        children: [
          ...
          SizedBox(
            ...
            child: benchImp != null
              ...
              decoration: BoxDecoration(
                color: _getImpColor(benchImp.imp),
                borderRadius: BorderRadius.circular(6),
              ),
              child: Text(
                benchImp.imp > 0
                    ? '+${benchImp.imp.toStringAsFixed(1)}'
                    : benchImp.imp.toStringAsFixed(1),
                style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                  color: Colors.white,
                  fontWeight: FontWeight.bold,
  ...
  PlayerStatImp? _getBenchImp(int playerStatId) {
    final imps = playerImps[playerStatId];
    if (imps == null) return null;

    try {
      return imps.firstWhere((imp) => imp.per == 'fullGame');
    } catch (e) {
      return null;
    }
  }
