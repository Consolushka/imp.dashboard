...
<template>
  ...
  <main v-else-if="matchData" class="max-w-[1280px] mx-auto p-lg lg:p-jumbo">
    <!-- Header Section -->
    <header class="flex flex-col mb-xl border-b-4 border-primary pb-md gap-sm">
      <h1 class="font-h1 text-h1 uppercase tracking-tighter truncate">{{ matchData.title }}</h1>
      <div class="flex flex-row justify-between items-center gap-md">
        <p class="font-label-caps text-label-caps text-neutral-medium uppercase flex-1 min-w-0 truncate">{{ matchData.subtitle }}</p>
    ...
    <section class="grid grid-cols-1 lg:grid-cols-12 gap-lg mb-jumbo">
      <div class="lg:col-span-8 bg-surface-white border-4 border-primary shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] p-lg relative overflow-hidden">
        ...
        <div class="hidden sm:flex items-center justify-around w-full">
          <!-- Home Team -->
        ...
        <!-- Mobile Scoreboard (Stacked) -->
        <div class="flex sm:hidden flex-col gap-lg w-full">
          <!-- Row 1: Teams and Status -->
          <div class="flex justify-between items-center px-2">
            <!-- Home Team -->
            <div class="flex flex-col items-center gap-2">
              <div class="w-16 h-16 bg-primary text-on-primary flex items-center justify-center border-2 border-primary">
                <span class="material-symbols-outlined text-3xl">sports_basketball</span>
              </div>
              <span class="font-label-caps text-xs uppercase tracking-tighter text-center max-w-[80px] leading-none">{{ matchData.homeTeam.name }}</span>
            </div>
            
            <span class="font-label-caps text-label-caps bg-primary text-on-primary px-4 py-1">FINAL</span>

            <!-- Away Team -->
            <div class="flex flex-col items-center gap-2">
              <div class="w-16 h-16 bg-surface-container border-2 border-primary flex items-center justify-center">
                <span class="material-symbols-outlined text-3xl">sports_basketball</span>
              </div>
              <span class="font-label-caps text-xs uppercase tracking-tighter text-center max-w-[80px] leading-none">{{ matchData.awayTeam.name }}</span>
            </div>
          </div>

          <!-- Row 2: Scores -->
          <div class="flex justify-center items-center gap-8 border-t-2 border-ghost-gray pt-md">
            <span class="font-display-hero text-6xl font-black" :class="{ 'text-status-positive': matchData.homeTeam.isWinner }">{{ matchData.homeTeam.score }}</span>
            <span class="font-h1 text-4xl text-neutral-medium">:</span>
            <span class="font-display-hero text-6xl font-black" :class="{ 'text-status-positive': matchData.awayTeam.isWinner }">{{ matchData.awayTeam.score }}</span>
          </div>
        </div>
      </div>
